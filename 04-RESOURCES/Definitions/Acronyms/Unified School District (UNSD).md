---
creation_date: 2025-12-13
modification_date: 2025-12-13
author: Jimmy Briggs <jimmy.briggs@noclocks.dev>
description: A school district serving students from kindergarten through 12th grade (K-12).
tags:
  - Status/Complete
  - Topic/GIS
  - Topic/Education
  - Type/Definition
  - Type/Acronym
aliases:
  - Unified School District
  - UNSD
---

# Unified School District (UNSD)
## Overview

**Unified School District (UNSD)** is a type of school district in the United States that serves students from kindergarten through 12th grade (K-12). Unlike areas with separate elementary and secondary districts, unified districts provide a continuous educational pathway under a single administrative structure.

## Key Concepts

**K-12 coverage** refers to serving all grade levels from kindergarten through high school graduation.
**Single administration** is the unified governance structure overseeing all schools in the district.
**Geographic boundary** is the defined area from which students are enrolled.
**NCES code** is the National Center for Education Statistics unique identifier for school districts.

## Comparison with Other District Types

| Type | Grades | Abbreviation |
|------|--------|--------------|
| Unified School District | K-12 | UNSD |
| Elementary School District | K-8 | [ELSD](Elementary School District (ELSD).md) |
| Secondary School District | 9-12 | [SCSD](Secondary School District (SCSD).md) |

## Census Data

The Census Bureau maintains school district boundaries in TIGER/Line files with attributes including:
- District name and GEOID
- Functional status
- Land and water area

## Appendix

*Created: 2025-12-13 | Modified: 2025-12-13*

### See Also

- [Elementary School District (ELSD)](Elementary School District (ELSD).md)
- [Secondary School District (SCSD)](Secondary School District (SCSD).md)
- [Topologically Integrated Geographic Encoding and Referencing (TIGER)](Topologically Integrated Geographic Encoding and Referencing (TIGER).md)

### Backlinks
<!-- dynamic content -->
